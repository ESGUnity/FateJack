...
{
    [Header("프리팹")]
    [SerializeField] GameObject prefab_UISkill;

    [Header("컴포넌트")]
    GameObject panel_SkillInfoBase;
    [HideInInspector] public GameObject layoutGroup_SkillInfoBase;
    ...
    public void AddSkillObject(S_Skill loot) 
    {
        GameObject go = Instantiate(prefab_UISkill);

        go.transform.SetParent(layoutGroup_SkillInfoBase.transform, false);
        go.GetComponent<S_UISkill>().SetSkillObjectInfo(loot);
        ownedSkillList.Add(go);

        UpdateTotalSkillCount();
    }
    public void RemoveSkillObject(S_Skill skill) 
    {
        GameObject skillGo = null;
        foreach (GameObject go in ownedSkillList)
        {
            if (go.GetComponent<S_UISkill>().SkillInfo.Equals(skill))
            {
                skillGo = go;
                break;
            }
    ...
    public void UpdateSkillObject() // 전리품 텍스트 및 ActivatedCount 업데이트
    {
        foreach (GameObject go in ownedSkillList)
        {
            go.GetComponent<S_UISkill>().UpdateSkillObject();
        }

        S_PlayerSkill.Instance.SetSiblingUISkills();
    }
    ...
    public void BouncingSkillObjectVFX(S_Skill skill)
    {
        foreach (GameObject go in ownedSkillList)
        {
            if (go.GetComponent<S_UISkill>().SkillInfo == skill)
            {
                go.GetComponent<S_UISkill>().BouncingVFX();
            }
        }
    }
}

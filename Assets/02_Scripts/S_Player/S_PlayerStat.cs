...
// NOTIFY : 절대 스탠드 후 카드를 내는 기믹은 만들지 마라... ㅅ제외도!!!!!!!!!!!!!!!!!
// NOTIFY : 항상 피해와 창조가 공존한다면 항상 피해를 먼저 주고 창조 인도 역류해라!!!!!!!!!!!!
...
public class S_PlayerStat : MonoBehaviour
{
    ...
    void CalcStatByLoot(S_Card hitCard) // 조건과 수식에 따라서 전리품을 단지 트리거만 하는 용도
    {
        //List<S_Card> stacks = pCard.GetStackCards();

        //// 전리품 전용 범람용(전리품의 범람은 히트한 카드가 아니기에 조금 다르게 계산된다.)
        //int count = 0;
        //bool canOverflow = false;

        //// 메아리, 전리품 전용 범람, 대미장식, 성장형, 조건형 확인
    ...
    // 각종 스탯 MinMax 제한
    public void CheckStatsMinMaxValue() // 체력 및 의지 계산
    {
        // 체력
        currentHealth = MaxHealth - standDamagedHealth + additionalHealth;
    ...
    public void ActiveVFXCollection() // 각종 수치(능력치, 숫자 합, 한계, 피조물 체력, 전리품 등) 변화 VFX
    {
        S_StatInfoSystem.Instance.ChangeStatVFX();
        S_StatInfoSystem.Instance.ChangeSpecialAbility();
